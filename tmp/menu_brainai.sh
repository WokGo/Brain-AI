...
clear
echo -e "${GREEN}🧠 Brain-AI 통합 관리 콘솔 (v5.3)${NC}"
echo "----------------------------------------"
echo "실행할 작업을 선택하세요:"
echo ""
...
case $choice in
  ...
  4)
    ...
    cat > .github/workflows/brainai-ci.yml <<'EOF'
name: Brain-AI CI/CD
    ...
    steps:
      ...
      - name: Build and run stack
        run: |
          ...
          docker ps -a
EOF
    echo -e "${GREEN}✅ GitHub Actions 파일 생성 완료 (.github/workflows/brainai-ci.yml)${NC}"
    ;;
  5)
